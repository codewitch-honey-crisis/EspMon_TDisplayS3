...
static lv_font_t * tiny_ttf_create(const char * path, const void* data, size_t data_size,  lv_coord_t line_height)
{
...
lv_font_t* tiny_ttf_create_file(const char * path, lv_coord_t line_height) {
    return tiny_ttf_create(path,NULL,0,line_height);
}
#endif
lv_font_t* tiny_ttf_create_data(const void* data, size_t data_size, lv_coord_t line_height) {
    return tiny_ttf_create(NULL,data,data_size,line_height);
}
